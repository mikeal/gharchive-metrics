...
module.exports.year = async argv => {
  ...
  while (months.length) {
    ...
    console.log(`Service generated ${ret.length} filtered files for ${month}. Downloading...`)
    last = download.all(argv.profile, ret, argv.outputDir, 100, services)
    last.then(downloaded => {
      services = downloaded.services
